...
"""
from token_meter import CounterCore, count_tokens, token_meter, PRICE
import gh_models
import router
import compressor
...
async def eval_compression(text: str, task: str, large_model: str):
    ...
    saved_cost = round((bt - at) / 1000 * PRICE["LARGE"], 6)
    return {
...
async def eval_routing(prompts: list[str], large_model: str):
    ...
    return {"rows": rows, "before": before_core.snapshot(), "after": after_core.snapshot(),
