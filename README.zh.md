...
所用模型(经由 GitHub Copilot)。计价遵循 **GitHub Copilot 模型价目表**
(每 1M tokens 消耗的 Credits, `1 credit = $0.01`), 每个模型统一单一费率 ——
不再区分个人/企业:

| 档位  | 模型             | 每 1M Credits(输入 / 输出) | 典型用途 |
|-------|-----------------|-----------------------------|----------|
| LARGE | `gpt-5.5`       | `500` / `3000`              | Agent、代码生成、多步推理 |
| LARGE | `claude-opus-4.8` | `500` / `2500`            | Agent、代码生成、多步推理 |
| MID   | `gpt-5.4-mini`  | `75` / `450`                | 对话、摘要、抽取 |
| TINY  | `gpt-5-mini`    | `25` / `200`                | 分类、关键词/规则匹配 |

成本直接按各模型的输入/输出 credit 费率计算, 再换算为 USD(`credits × $0.01`)。
完整模型费率表见 `backend/token_meter.py`(`MODEL_CREDITS_PER_1M`)。
...
```
INCOMING REQUEST
        └─ Prompt < 500 tokens?  ── 是 ─→ TINY   (分类 · 抽取)
                                                                                                         └─ 否 ─→ 需要多步推理?
                                                                                                                                                         ├─ 否 ─→ MID   (对话 · 摘要)
                                                                                                                                                         └─ 是 ─→ LARGE (Agent · 代码)
```
...
Token 用 `tiktoken` 计数(与具体厂商无关),并按档位累计成本。若启动时无法拉取
`cl100k_base` 编码文件,应用会自动降级为离线近似计数,保证服务仍可启动。
